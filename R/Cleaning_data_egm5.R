...
# ════════════════════════════╗
#                             ▼
flux1 <- read_delim("raw_data/egm5_raw_test.txt")
flux1 <- flux1 %>% # Here the "start," "end," and "zero" lines are cut and all the columns are processed to match either as a numeric or character value
  filter(!is.na(CO2)) %>%
  mutate(across(c(Plot_No, CO2, Pressure, Flow, H2O, Tsen, O2, PAR, Tsoil, Tair, Msoil, "...18", "...19", "...20", Flux, "...22"), as.numeric)) %>%
  mutate(across(c("Tag(M3)", Rec_No, Error, Aux_V), as.character))
flux1 <- flux1 %>%
...
#                             ▼
# flux2 <- read_delim("raw_data/egm5_raw_test.txt")
# flux2 <- flux1 %>% # Here the "start," "end," and "zero" lines are cut and all the columns are processed to match either as a numeric or character value
#   filter(!is.na(CO2)) %>%
#   mutate(across(c(Plot_No, CO2, Pressure, Flow, H2O, Tsen, O2, PAR, Tsoil, Tair, Msoil, "...18", "...19", "...20", Flux, "...22"), as.numeric)) %>%
#   mutate(across(c("Tag(M3)", Rec_No, Error, Aux_V), as.character))
# flux2 <- flux2 %>%
...
if (nrow(fluxdata_2min)*ncol(fluxdata_2min)>0){
  ...
  # ════════════════════════════════════╗
  #                                     ▼
  write_delim(fluxdata_2min,"clean_data/EGM5_Test_clean_2min.dat", delim = "\t")
}
#
# 5min measurements
#
...
if (nrow(fluxdata_5min)*ncol(fluxdata_5min)>0){
  ...
  # ════════════════════════════════════╗
  #                                     ▼
  write_delim(fluxdata_5min,"clean_data/EGM5_Test_clean_5min.dat", delim = "\t")
}
#
# 10min measurements
#
...
if (nrow(fluxdata_10min)*ncol(fluxdata_10min)>0){
  ...
  # ═════════════════════════════════════╗
  #                                      ▼
  write_delim(fluxdata_10min,"clean_data/EGM5_Test_clean_10min.dat", delim = "\t")
}
#
# 15min measurements
#
...
if (nrow(fluxdata_15min)*ncol(fluxdata_15min)>0){
  ...
  # ═════════════════════════════════════╗
  #                                      ▼
  write_delim(fluxdata_15min,"clean_data/EGM5_Test_clean_10min.dat", delim = "\t")
}
#
...
if (nrow(fluxdata_left)*ncol(fluxdata_left)>0){
  ...
  # ════════════════════════════════════╗
  #                                     ▼
  write_delim(fluxdata_left,"clean_data/EGM5_Test_clean_left.dat", delim = "\t")
}
#
# The End